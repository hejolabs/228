...
export default function Students() {
  ...
  const fetchStudents = async () => {
    let url = '/api/students?enrollment_status=active'
    if (filterGroup !== 'all') url += `&class_group_id=${filterGroup}`
  ...
  return (
    <div>
      ...
      <div className="flex gap-3 mb-4">
        ...
        <p className="text-sm text-muted-foreground self-center">수업중인 학생만 표시됩니다.</p>
      </div>
      ...
      <Dialog open={open} onOpenChange={setOpen}>
        <DialogContent>
          <DialogHeader>
            <DialogTitle>{editId ? '학생 정보 수정' : '학생 등록'}</DialogTitle>
          </DialogHeader>
